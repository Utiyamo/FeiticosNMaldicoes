// routes/builder/specialization/controlador.tsx
import {
  Form,
  useLoaderData,
  useActionData,
...
import type { Route } from './+types/controlador';
...
const REQUIRED_SKILLS = ['Percepção', 'Persuasão'] as const;
...
export async function loader({ request }: Route.LoaderArgs) {
  ...
  if (savedData.specialization !== 'Controlador') {
...
export async function action({ request }: Route.ActionArgs) {
  ...
  // ✅ VALIDAÇÃO: não pode repetir craft (sem prefixo), Percepção ou Persuasão
  const craft = formData.get('craft') as string;
  const craftBase = craft?.replace('Ofício (', '').replace(')', '') ?? '';
  const forbidden = new Set([craftBase, 'Percepção', 'Persuasão']);
  ...
  if (invalidFree.length > 0) {
    return {
      errors: {
        freeSkills: [
          `As perícias livres não podem repetir ${craftBase}, Percepção ou Persuasão.`,
        ],
      },
      submitted: Object.fromEntries(formData.entries()),
    };
  ...
  // ✅ Só 1 parse com type: 'Controlador'
...
export default function ControladorDetail() {
      ...
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-emerald-400 mb-2">
            Especialização: <span className="text-emerald-300">Controlador</span>
          </h1>
          <p className="text-gray-400">
            Domina invocações com maestria — estratégico, criativo e capaz de controlar o campo de batalha.
          </p>
        </div>
        ...
        <div className="bg-gray-800 rounded-xl p-6 shadow-lg border border-emerald-700/30">
          <div className="mb-6 p-4 bg-emerald-900/20 rounded-lg">
            <h3 className="font-bold text-emerald-300 mb-2">Características (p.90)</h3>
            <ul className="list-disc pl-5 space-y-1 text-sm">
              <li><strong>PV inicial:</strong> 10 + mod. CON</li>
              <li><strong>PE inicial:</strong> 5 + mod. INT/SAB</li>
              ...
              <ul className="list-none pl-4 mt-1 space-y-1">
                ...
                <li>• <strong>Perícias:</strong> Ofício, Percepção, Persuasão + 2 livres</li>
              </ul>
              <li><strong>Atributo-chave:</strong> Inteligência <em>ou</em> Sabedoria</li>
              <li><strong>Habilidades Base:</strong> Treinamento em Controle (invocar 2 criaturas, manter 2 ativas)</li>
            </ul>
          </div>
          ...
          <Form method="post" className="space-y-8">
            ...
            <div>
              ...
              <p className="text-sm text-gray-400 mb-2">
                Escolha <strong>2 perícias diferentes</strong>, exceto Ofício, Percepção e Persuasão.
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div className="flex justify-between pt-6">
              ...
              >
                {isSubmitting ? 'Salvando...' : 'Confirmar Controlador →'}
              </button>
            </div>
          </Form>
        </div>
